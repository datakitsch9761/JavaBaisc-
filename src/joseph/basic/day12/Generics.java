...
public class Generics {
    public static void main(String[] args) {
        //제네릭 Generic
        //다양한 테이터 타임의 객체를 다루는 메서드나
        //컬렉션 클래스에 컴파일할때 자료형 검사를 해주는 기능

        //즉, 클래스에 사용할 타입의 정보를 동적으로 넘겨줄 수 있고
        //실행할때에 발생할 수 있는 타입간 오류를 컴파일할때에
        //발견할 수 있도록 해주는 기능
        //JDK 1.5부터 추가된 기능
...
//                (int)numbers2[0] + (int)numbers2[1] + (int)numbers2[2]);
        ...
        //System.out.println(Integer.parseInt((int)numbers2[0]) +
        //Integer.parseInt((int)numbers2[1]) +
        //Integer.parseInt((int)numbers2[2]));
        //오류발생 -> 비정상 형변환이 원인
        //컴파일할때 잘못되었다고 오류를 띄워줌
        // =>Object타입은 바로 정수로 변환 불가라서

        //JDK 1.5이전에서는 여러 타입을 사용하는 대부분의 클래스나
        //메서드에서 인수나 반환값으로 object타입을 사용했었음
        //이런경우, 반환된 object 객체를 다시 원하는 타입으로 변환해야 하고
        //운이 나쁜 경우, 오류가 발생할 수도 있었음
