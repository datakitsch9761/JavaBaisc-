...
public class JavaString {
    public static void main(String[] args) {
        ...
        //자바에서 String 객체는 메모리 재활용 Interning 이라고 한다
        //즉 생성하는 String 객체값을 String Pool에 저장하는데,
        //그 다음에 생성한 String 객체의값이 이전과 유사한 경우
        ...
        //값이 저장된 위치를 비교한다 (참조값 비교_
        if(str1 == str2)
            System.out.println("str1과 str2가 저장 위치가 같음");

        if(str1 == str3)
            System.out.println("str1과 str3 저장위치가 같음");
        ...
        //concat : 두문자열을 하나로 이어 묶음
        //문자열에 + 연산자를 사용하면 먼저 StringBuildf로 변환한 후
        ...
        //보통, 문자열내에 특정 문자가 포함되어 있는지 알아낼때 사용
        ...
        //시작위치를 재조정하기 위해 fromIndex 매개변수를 사용한다
        ...
        //replace 메서드를 이용하면 가운데 있는 공백도 제거가 가능하다


        //length ; 문자열의 길이(갯수)를 반환하는 메서드
        //문자열에 실제 바이트수를 알아내려면 getByte를 사용해야한다
        ...
        //ex) hello, java!! 라는 문자열에서 각 단어의 첫 글자를
        //대문자로 만드는 코드를 작성해보세요
        String l = "hello, java!!";
        String[] words = l.split(" ");
        System.out.println(
        (words[0].charAt(0) + "").toUpperCase(Locale.ROOT));

        System.out.println(
        (words[1].charAt(0)+ "").toUpperCase(Locale.ROOT));
        ...
        String word1 = (words[0].charAt(0) + "").toUpperCase(Locale.ROOT);
        String word2 = (words[1].charAt(0)+ "").toUpperCase(Locale.ROOT);

        l = l.replace("h",word1);
        l = l.replace("j",word2);
        System.out.println(l);

        //substring : 문자열의 시작 취치부터 끝위치 -1 까지의
            //문자열을 추출하는 메서드
            //substring(시작)
            //substring(시작,끝 -1)

        //ex) 주민코드에서 생년월일을 추출해 봄
        System.out.println(jumin.substring(0,6)); // 0 ~ 5까지의 문자를 추출한다

        //ex) 주민코드에서 생년월일을 제외한 나머지 추출해 봄
        System.out.println(jumin.substring(7,14));// -> 방법 1
        System.out.println(jumin.substring(7)); // 뒤에 숫자를 생략하면 끝의 위치를 자동으로 지정!!

        //valueOf : 숫자를 문자형을 변환시 사용하는 메서드
        int height = 123;
        System.out.println(height + "");
        System.out.println(String.valueOf(height));
        System.out.println(Integer.toString(height)); //숫자를 문자형
       ...
       // %ts : second
        ...
        //String객체는 불변 객체이므로
        //새로운 문자열이 추가되면 새로운 공간을 생성한 후 추가함 -> 메모리 낭비 심함
        //StringBuilde나 StringBuffer를 이용하면 기존 공간에 새로운 문자열을 바로 추가할 수 있으므로
        //메모리를 절약할 수 있음
        //따라서 + 를 이용해서 문자열을 결합하는 것보다는
        //StringBuilder를 이용한는 것이 더 나음;
        //오해의 소지를 줄이기 위해서 무조건 StringBuilder 객체를 부를때에는 "toString()" 입력필수!!
