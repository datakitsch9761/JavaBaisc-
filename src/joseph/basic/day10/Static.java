...
public class Static {
    public static void main(String[] args) {
        ...
        //한번 값이 생성어 저장되면 모든 영역에서 값을 공유하는 변수

        //객체마다 값을 개별적으로 관리해야 할 필요가 없고
        //모든 클래스가 공유해야 하는 값을 유지하기 위해 사용
        //static 이라는 키워드로 선언해야 함(실무에서는 잘 사용하지 않는다)
        //클래스 변수는 객체의 생성 과정없이 바로 사용가능
...
class Member{
    static int allOfMember; //클래스변수 -> 공유변수 다른 객체와 값을 공유한다
    String userid; //인스턴스 변수
    String passwd; //인스턴스 변수
}
